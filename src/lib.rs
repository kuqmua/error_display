        ...
        syn::Data::Struct(_) => quote::quote! {
            if CONFIG.is_show_source_place_enabled && CONFIG.is_show_github_source_place_enabled {
                write!(
                    ...
                    self.where_was.github_file_line_column(&crate::once_cell_globals::git_info::GIT_INFO.data),//_with_readable_time
                    self.source
                )
                    ...
                    #ident::#variant { source, where_was } => {
                        if CONFIG.is_show_source_place_enabled && CONFIG.is_show_github_source_place_enabled
                        {
                            write!(
                                ...
                                where_was.github_file_line_column(&crate::once_cell_globals::git_info::GIT_INFO.data),//_with_readable_time
                                source
                            )
                        } else if CONFIG.is_show_source_place_enabled {
                            write!(
